...
func TestIssue77273(t *testing.T) {
	...
	tmpdir := mktmpdir(t)
	defer os.RemoveAll(tmpdir)
	compile(t, "testdata", "issue77273.go", filepath.Join(tmpdir, "testdata"), nil)

	fset := token.NewFileSet()
	pkg, err := Import(fset, make(map[string]*types.Package), "./testdata/issue77273", tmpdir, nil)
	if err != nil {
		t.Fatal(err)
	}
	...
	named, ok := streamObj.Type().(*types.Named)
	if !ok {
